...
@section('content')
    ...
    <div class="container-fluid page-header py-5">
        <h1 class="text-center text-white display-6">Riwayat Pemesanan</h1>
    ...
    <div class="container-fluid fruite py-5">
        <div class="container py-5">
            ...
            <form id="menu-form" action="{{ route('orders') }}" method="GET">
                <div class="row g-4">
                    <div class="col-lg-12">
                        <div class="row g-4">
                            <div class="col-xl-3">
                                <div class="input-group w-100 mx-auto d-flex">
                                    <input type="search" class="form-control p-2" placeholder="Cari no pesanan"
                                ...
                                <div class="bg-light ps-3 py-3 rounded d-flex justify-content-between mb-4">
                                    ...
                                    <select name="sort_by" class="border-0 form-select-sm bg-light me-3"
                                        ...
                                        <option value="tanggal_terawal"
                                            {{ request()->input('sort_by') == 'tanggal_terawal' ? 'selected' : '' }}>Tanggal Terawal</option>
                                            ...
                                            <ul class="list-unstyled fruite-categorie" style="margin-right: 40px;">
                                                <li>
                                                    <div class="d-flex justify-content-between fruite-name">
                                                        <a href="{{ route('orders', array_merge(request()->except(['status', 'page']), ['status' => 'null'])) }}"
                                                    ...
                                                    @foreach ($status_pemesanan as $key => $status)
                                                        <div class="d-flex justify-content-between fruite-name">
                                                            <a href="{{ route('orders', array_merge(request()->except(['status', 'page']), ['status' => $key])) }}"
                                                                class="{{ request()->has('status') && request()->status == $key ? 'active' : '' }}"><i
                                                                    class="fas fa-apple-alt me-2"></i> {{ $status }}</a>
                                                            <span></span>
                                                        </div>
                                                    @endforeach
                                                </li>
                                        ...
                                        @foreach ($orders as $order)
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td style="text-align: center"> <a href="{{ route('orders.detail', $order->id) }}"
